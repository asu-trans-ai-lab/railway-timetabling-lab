...
supercolumn closes to zero; branch-and-price reaches $[26,29]$ but cannot certify seed1 with departure-window branching alone (12 departure-inseparable subtrees) --- precedence branching is required; instances with
...
C2\_L6\_n8\_seed21 (double track) & 8 & 544 & \textbf{49} & proven (0.1 s, after direction fix) \\
\bottomrule
\end{tabular}
\end{table}
...
seed1 & B6 B\&P & 29 & 26 & no (12 stalled subtrees) & 164.9 \\
\midrule
...
bounds on the open tier (node CG iteration-capped $\Rightarrow$ not valid bounds); (b) the seed1 B\&P ``proof'' (stalled subtrees had been discarded; honest result $[26,29]$, unproven); (c) the seed3
...
Eight trains on the same corridor length: single track $[105,138]$ vs double track \textbf{49 (proven)} (an earlier interval $[39,43]$ was INVALID: the CP-SAT model could traverse one-way tracks backward; fixed and re-proven) ---
