...
from .trajectory_nogood import Trajectory,TrajectoryNoGood,trajectory_context
...
def build(edge,*,paths,trains,arcs,mow,config,domains,parent_id,proof_scope,validation_args):
    from validator import trusted_physics
    ...
    sha=hashlib.sha256(Path(trusted_physics.__file__).read_bytes()).hexdigest()
